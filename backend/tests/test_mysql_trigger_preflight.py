...
def test_preflight_allows_triggers_when_binary_log_trust_is_enabled(migration) -> None:
    bind = Bind({"SELECT DATABASE()": "masri_migr_audit", "@@GLOBAL.log_bin": (1, 1)})
...
def test_preflight_blocks_when_binary_logging_would_reject_trigger(migration) -> None:
    bind = Bind(
        {
            "SELECT DATABASE()": "masri_migr_audit",
            "@@GLOBAL.log_bin": (1, 0),
            "SHOW GRANTS": ["GRANT TRIGGER ON `masri_migr_audit`.* TO `app`@`%`"],
        }
    )
    ...
    bind = Bind(
        {
            ...
            "SHOW GRANTS": [
                ...
                "GRANT SELECT, TRIGGER ON `masri_migr_audit`.* TO `app`@`%`",
            ],
        }
    )
