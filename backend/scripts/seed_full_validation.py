...
DATABASE_URL whose file name is not of the ``masri_full_validation_*.db`` form, so a
mistyped argument cannot reseed ``masri_preview.db`` or ``masri_store_dev.db``.
...
copy always produces the identical dataset.
    ...
    copy backend\\data\\masri_preview.db backend\\data\\masri_full_validation_<stamp>.db
    DATABASE_URL=sqlite+pysqlite:///./data/masri_full_validation_<stamp>.db alembic upgrade head

    # 2. seed it
    python scripts/seed_full_validation.py \
        --database-url sqlite+pysqlite:///./data/masri_full_validation_<stamp>.db
"""
...
EMAIL_DOMAIN = "acceptance.invalid"
...
VALIDATION_DB_PATTERN = re.compile(r"masri_full_validation_[0-9]{8}-[0-9]{6}\.db$")
...
def _guard_database_url(url: str) -> None:
    ...
    if not VALIDATION_DB_PATTERN.search(tail):
        raise SystemExit(
            ...
            f"  got     : {url}\n"
            "  expected: .../data/masri_full_validation_<yyyymmdd-hhmmss>.db"
        )
...
def seed_store_settings(db: Session) -> None:
    ...
    if row is None:
        row = StoreSettings()
        db.add(row)
    row.store_name = row.store_name or "Acceptance Store"
