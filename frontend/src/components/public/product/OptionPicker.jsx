        ...
        <div key={axis.id} className="vs-options__axis" role="group" aria-label={axis.name}>
          ...
          <div className="vs-options__row">
            {axis.values.map((value) => (
              <button key={value.id} type="button" className="vs-option" aria-pressed={Number(simpleChoices[axis.id]) === Number(value.id)} onClick={() => onSimplePick(axis.id, value.id)}>{value.value}</button>
            ))}
          </div>
        </div>
      ))}
