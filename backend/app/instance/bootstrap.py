...
content must be imported separately, and admin creation is the separate
`app.initial_data` command. No customer catalog ships in source control.
...
def _settings_updates(row: StoreSettings, profile: InstanceProfile) -> dict[str, object]:
    ...
    desired: dict[str, object | None] = {
        ...
        "logo_url": profile.store.logo_url,
        "favicon_url": profile.store.favicon_url,
